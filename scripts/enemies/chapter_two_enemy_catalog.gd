...
static func create_gospel_card() -> CardData:
	return _make_card(
		"福音",
		"使用者对所有其他存活单位分别造成基础伤害 6 的智力伤害。每名实际受到正数伤害的单位获得 1 点咒波，并将 1 张辐照咒害洗入其牌库。然后使用者进入第二阶段：力量改为 6、敏捷改为 3、智力改为 1、生命上限改为 135、武器基础伤害改为 4、攻击范围改为 2，并将当前 AP 改为 0；当前生命不会因生命上限变化而回复。",
		0,
...
static func _get_spec(id: StringName) -> Dictionary:
	...
	var specs := {
		&"gray_shield_guard": {"name": "灰垒盾卫", "base_health": 17, "strength": 3, "agility": 2, "intelligence": 1, "weapon": {"name": "灰垒重盾", "damage": 2, "range": 1}, "military": 6, "mutation": 1, "fields": ["rock_scale"], "tags": military, "ai_profile": EnemyAIProfile.Preset.LOW_HEALTH_GUARD, "art_index": 0, "trait": "回合开始时，若灰垒盾卫处于军阵，获得 4 护甲。"},
		&"holy_spearman": {"name": "圣枪兵", "base_health": 13, "strength": 3, "agility": 4, "intelligence": 1, "weapon": {"name": "圣垒长枪", "damage": 3, "range": 2}, "military": 6, "mutation": 1, "fields": ["lashing"], "tags": military, "art_index": 1, "trait": "每回合第一次武器打击结算前，若圣枪兵处于军阵，且目标与圣枪兵的距离等于其当前攻击范围，该打击获得 +2 伤害加值。无论是否满足条件，本回合后续武器打击均不再触发。"},
		...
		&"field_priest": {"name": "战地司祭", "base_health": 17, "strength": 1, "agility": 3, "intelligence": 4, "weapon": {"name": "战地圣杖", "damage": 2, "range": 3, "damage_type": CardEnums.DamageType.INTELLIGENCE, "range_type": EquipmentData.WeaponRangeType.RANGED}, "military": 6, "mutation": 1, "fields": ["mud_lung"], "tags": military, "art_index": 3, "trait": "回合开始时，若战地司祭处于军阵，使每名与其相邻的存活军事友军回复 2 生命。没有合法友军时不执行。"},
		&"punishment_knight": {"name": "刑罚骑士", "base_health": 16, "strength": 4, "agility": 4, "intelligence": 1, "weapon": {"name": "刑罚巨刃", "damage": 4, "range": 1}, "military": 6, "mutation": 2, "fields": ["stampede"], "tags": military, "ai_profile": EnemyAIProfile.Preset.LOW_HEALTH_BERSERK, "art_index": 4, "trait": "回合开始时，若刑罚骑士处于军阵，使其下一次非强制移动的移动距离 +2。该加值在完成该次移动后移除；未移动时不会自动到期。"},
		&"standard_bearer": {"name": "执旗官", "base_health": 17, "strength": 2, "agility": 3, "intelligence": 3, "weapon": {"name": "圣旗辉光", "damage": 2, "range": 2, "damage_type": CardEnums.DamageType.INTELLIGENCE}, "military": 6, "mutation": 1, "fields": ["enlightenment"], "tags": military, "art_index": 5, "trait": "回合开始时，从相邻的存活军事友军中选择当前生命最低的一名；该友军获得 3 护甲并抽 1 张牌。没有合法友军时不执行。"},
		&"holy_bastion_commander": {"name": "圣垒军团长", "rank": EnemyEnums.EnemyRank.ELITE, "base_health": 28, "strength": 4, "agility": 4, "intelligence": 2, "weapon": {"name": "军团长战戟", "damage": 4, "range": 2}, "military": 6, "mutation": 2, "fields": ["empty_eye"], "tags": military + ["commander"], "art_index": 6, "trait": "锁定意图时，圣垒军团长依战斗轮次交替公开“固守”或“推进”，并在下次行动开始时先执行。固守：军团长及所有处于军阵的存活军事友军各获得 4 护甲。推进：这些单位的下一次移动距离加值至少变为 +1，并使其下一次攻击获得 +2 伤害加值。每有一名与军团长相邻的存活军事友军，军团长获得 1 点伤害减免，至多 2 点。"},
		...
		&"gray_bastion_paladin": {"name": "灰垒圣骑", "rank": EnemyEnums.EnemyRank.BOSS, "base_health": 45, "strength": 5, "agility": 4, "intelligence": 2, "weapon": {"name": "圣骑重剑", "damage": 4, "range": 1}, "military": 8, "tags": military + ["boss"], "art_index": 10, "trait": "战斗开始时拥有杯、盾、翼、剑 4 枚圣徽：杯使回合结束回复 4 生命；盾提供 2 点伤害减免；翼使最大 AP 和移动距离各 +1；剑使每回合第一次武器打击获得 +3 伤害加值。凯旋圣像生命降至 30、20、10、0 时依次移除杯、盾、翼、剑。灰垒圣骑首次受到致命伤害时保留 1 生命并排队合体；结算时若仍存活，清除原状态、护甲和全部牌区，变为军神圣骸并继承剩余圣徽。其当前生命和生命上限改为 60 加仍存活圣像的当前生命；若圣像对象仍存在，则移动到其所在格并移除圣像，没有圣像时原地变身。"},
		&"triumph_statue": {"name": "凯旋圣像", "rank": EnemyEnums.EnemyRank.BOSS, "base_health": 40, "strength": 0, "agility": 0, "intelligence": 0, "weapon": {"name": "静默圣徽", "damage": 0, "range": 0}, "tags": ["support", "statue", "boss"], "art_index": 11, "trait": "凯旋圣像可以成为攻击目标，但不进入行动顺序。每次受到伤害后，生命降至 30、20、10、0 时分别熄灭灰垒圣骑的杯、盾、翼、剑圣徽；一次伤害跨越多个阈值时，依次熄灭所有被跨越的圣徽。"},
		...
		&"flesh_spawn": {"name": "血肉衍生物", "base_health": 1, "fixed_max_health": 1, "strength": 2, "agility": 3, "intelligence": 2, "weapon": {"name": "血肉齿爪", "damage": 1, "range": 1}, "tags": ["summon"], "art_index": 8, "trait": "由魔血构造体召唤时，血肉衍生物拥有固定 1 点生命上限，与召唤者共享牌库和弃牌堆，但拥有独立手牌；召唤后从共享牌库抽 2 张牌。"},
	}
