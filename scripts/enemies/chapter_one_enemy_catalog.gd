...
static func _get_spec(id: StringName) -> Dictionary:
	var specs := {
		...
		&"kraken": {"name": "克拉肯", "rank": EnemyEnums.EnemyRank.ELITE, "base_health": 48, "strength": 3, "agility": 2, "intelligence": 3, "weapon": {"name": "群腕", "damage": 2, "range": 2}, "basic": 8, "mutation": 2, "art_index": 7, "trait": "回合开始时，若克拉肯当前生命高于生命上限的 66%，额外抽 2 张牌并将当前 AP 乘 3；否则若高于 33%，额外抽 1 张牌并将当前 AP 乘 2；否则当前 AP 不变。"},
